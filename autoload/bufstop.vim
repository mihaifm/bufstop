...
function! s:BufstopGetBufferInfo()
    redir => s:lsoutput
    exe "silent ls"
    redir END
...
" wrapper for s:Bufstop(), default mode
function! bufstop#slow()
  let s:fast_mode = 0
  let s:preview_mode = 0
  call s:Bufstop()
...
" wrapper for s:Bufstop(), fast mode
function! bufstop#fast()
  let s:fast_mode = 1
  let s:preview_mode = 0
  call s:Bufstop()
  call s:UnmapPreviewKeys()
...
" wrapper for s:Bufstop(), preview mode
...
function! bufstop#switch_to(bufidx)
...
function! s:BufstopSpeedMount()
  ...
  for key in g:BufstopSpeedKeys
    ...
    exe "nmap <silent> " . combo . " " . ":call bufstop#switch_to(" . idx . ")<CR>"
    let idx += 1
  endfor
...
function! bufstop#statusline_fast()
  let s:use_statusline = 1
...
function! bufstop#mode_fast_start()
  let s:bufstop_mode_fast = 1
  call s:BufstopModeInit()
endfunction

" start BufstopMode in preview mode
function! bufstop#mode_start()
  let s:bufstop_mode_fast = 0
  call s:BufstopModeInit()
endfunction
